...
echo Creating initiator cluster specific certs 01 ..
...
echo Creating target cluster specific certs 02 ..
...
echo Creating target cluster specific certs 03 ..
openssl req -new -newkey rsa:4096 -sha256 -nodes -out cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.csr -keyout cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.key -subj "/C=FI/ST=`date +"%Z"`-`cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 3 | head -n 1`-`cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 3 | head -n 1`-`cat /dev/urandom | tr -dc 'a-z0-9' | fold -w 3 | head -n 1`/L=$mycls3/O=rootCA-SIQ/O=PowerScale/CN=Millenial@Support.LoCaL"
openssl x509 -days 3650 -req -in cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.csr -CA rootCA-SIQ-exp-`date -v+10y +%d-%B-%Y`.crt -CAkey rootCA-SIQ-exp-`date -v+10y +%d-%B-%Y`.key -set_serial 01 -out cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.crt
openssl x509 -in cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.crt -outform PEM -out cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.pem
openssl verify -CAfile rootCA-SIQ-exp-`date -v+10y +%d-%B-%Y`.pem cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.pem
...
#isi sync certificates peer import --certificate-path=cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.pem  --name=peer-cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`
...
#isi sync certificates peer import --certificate-path=cls-$mycls2-exp-`date -v+10y +%d-%B-%Y`.pem  --name=peer-cls-$mycls2-exp-`date -v+10y +%d-%B-%Y`
#isi sync certificates server import --certificate-path=cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.pem --certificate-key-path=cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.key --name=server-cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`
#isi sync settings modify --cluster-certificate-id=`isi sync certificates server list -v|egrep -i "ID:"|awk '{print $2;}'`

#isi certificate authority import --name=rootCA-SIQ --certificate-path=rootCA-SIQ-exp-`date -v+10y +%d-%B-%Y`.pem
#isi sync certificates peer import --certificate-path=cls-$mycls1-exp-`date -v+10y +%d-%B-%Y`.pem  --name=peer-cls-$mycls1-exp-`date -v+10y +%d-%B-%Y`
#isi sync certificates peer import --certificate-path=cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`.pem  --name=peer-cls-$mycls3-exp-`date -v+10y +%d-%B-%Y`
#isi sync certificates server import --certificate-path=cls-$mycls2-exp-`date -v+10y +%d-%B-%Y`.pem --certificate-key-path=cls-$mycls2-exp-`date -v+10y +%d-%B-%Y`.key --name=server-cls-$mycls2-exp-`date -v+10y +%d-%B-%Y`
#isi sync settings modify --cluster-certificate-id=`isi sync certificates server list -v|egrep -i "ID:"|awk '{print $2;}'`
